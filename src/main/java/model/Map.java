...
public class Map {
    ...
    private void populateMap() {
        TileType[][] tiles = new TileType[map.length][map.length];
        //populates with whatever terrain you want
        //this could be made better if you make map give you an array of
        //TileTypes instead
        Random rand = new Random();
        for (int r = 0; r < map.length; r++) {
            TileType[] row = new TileType[map.length];
            for (int c = 0; c < map.length; c++) {
                int seed = rand.nextInt(2);
                int value = seed + r + c;
                TileType type = TileType.PLAINS;
                if (value < 5 * map.length / 10) {
                    type = TileType.WATER;
                }
                if (value > 8 * map.length / 10 && value < 14 * map.length / 10
                        && c < 5 * map.length / 10 && r > 5 * map.length / 10) {
                    type = TileType.DESERT;
                } else if (value > 10 * map.length / 10
                        && value < 16 * map.length / 10) {
                    type = TileType.FOREST;
                }
                if (value > 14 * map.length / 10
                        && value < 18 * map.length / 10) {
                    type = TileType.MOUNTAIN;
                } else if (value >= 18 * map.length / 10) {
                    type = TileType.ICE;
                }
    ...
    public static void addEnemies(Bandit bandit, int numEnemies) {
        for (int i = 0; i < numEnemies; i++) {
            int r = new Random().nextInt(map.length);
            int c = new Random().nextInt(map.length);
            putSettlement("Bandit Hideout", bandit, map.length - 1, 0);
            while (!map[r][c].isEmpty()) {
                //try to get new coords
                r = new Random().nextInt(map.length);
                c = new Random().nextInt(map.length);
            }
