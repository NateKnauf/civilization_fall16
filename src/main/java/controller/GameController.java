...
public class GameController {
    ...
    public static void setLastClicked(TerrainTileFX last) {
        ...
        if (state == GameState.ATTACKING) {
            if (!attack(last)) {
                ...
                alert.setTitle("Attack Error");
                alert.setHeaderText("You can't attack like that!");
                alert.setContentText("Try a different thing!");
            ...
            if (move(last)) {
                ...
                alert.setContentText("Try moving somewhere else!");
    ...
    private static boolean nearSettlement(TerrainTileFX tile) {
        ...
        for (int r = row - 1; r < row + 2; r++) {
            for (int c = col - 1; c < col + 2; c++) {
                if (c >= 0 && c < GridFX.getSize()
                        && r >= 0 && r < GridFX.getSize()) {
    ...
    public static void ai() {
        ...
        for (int r = 0; r < GridFX.getSize(); r++) {
            for (int c = 0; c < GridFX.getSize(); c++) {
        ...
        if (banditHideout != null) {
            if (rand.nextInt(100) < GridFX.getSize()) {
        ...
        for (TerrainTile tile : bandits) {
            ...
            if (newRow >= 0 && newRow < GridFX.getSize()
                    && newCol >= 0 && newCol < GridFX.getSize()) {
    ...
    private static TerrainTile getRandomAdjacentTile(int row, int col) {
        ...
        while (!(r >= 0 && r < GridFX.getSize()
                && c > 0 && c < GridFX.getSize())) {
